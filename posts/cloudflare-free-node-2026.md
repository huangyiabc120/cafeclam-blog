...
用 Cloudflare 的免费服务，配合免费域名和开源脚本，就能以几乎 0 成本 实现自建节点，而且不限流量、永久可用。
...
- Cloudflare 账号 → [点击注册](https://dash.cloudflare.com/)
- 一个免费域名（推荐 eu.org 等免费后缀）
- 生成 UUID（用于节点配置的唯一标识）→ [在线生成](https://www.wetools.com/uuid)
- GitHub 账号（用于部署开源脚本）
...
- 源码：am-cf-tunnel
- GitHub：[amclubs/am-cf-tunnel](https://github.com/amclubs/am-cf-tunnel)
...
- **Windows**：v2rayN → [下载](https://github.com/2dust/v2rayN/releases)
- **Android**：v2rayNG → [下载](https://github.com/2dust/v2rayNG/releases)
- **macOS / iOS**：Shadowrocket / Stash → App Store 内搜索

---

## 注意事项

- 风险提示：Cloudflare 官方不允许将 Workers 用于搭建代理服务，有封号风险，请自行权衡
- TLS 加密：建议启用 TLS 节点，加密传输保障数据安全
- 优选 IP：配合本地优选 IP 工具使用，速度可以大幅提升
- 免费额度：Cloudflare 免费版每日有 10 万次请求额度，合理使用不会耗尽
- 域名托管：Pages 和 Workers 节点生效有延迟，建议绑定自定义域名后使用
